...
var app = express();
var PORT = 3e3;
var notebookDb = new Database("metamatrix.db");
notebookDb.exec(`CREATE TABLE IF NOT EXISTS notebooks (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, strategy_data TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
console.log("Local SQLite database initialized off-grid.");
var paypalClient;
try {
...
var db = new Database("metamatrix.db");
db.exec(`
...
app.post("/api/bridge/notebooklm", async (req, res) => {
  ...
  try {
    ...
    try {
      const insertPattern = notebookDb.prepare("INSERT INTO notebooks (title, strategy_data) VALUES (?, ?)");
      insertPattern.run(title || "Untitled Pattern", JSON.stringify(strategy));
      console.log(`[+] Saved locally: ${title || "Untitled Pattern"}`);
    } catch (err) {
      console.error("Local database write failed:", err);
    }
    const firestore2 = false;
    if (firestore2) {
      await firestore2.collection("notebooks").add({
